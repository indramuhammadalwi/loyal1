...
apt install curl tar wget clang pkg-config libssl-dev jq build-essential bsdmainutils git make ncdu gcc git jq chrony liblz4-tool -y
...
chmod 775 loyald
mv loyald /usr/local/bin/
rm loyal_v0.25.1_linux_amd64.tar.gz
...
# create service
tee /etc/systemd/system/$LYL.service > /dev/null <<EOF
[Unit]
Description=$LYL
...
systemctl enable $LYL
systemctl restart $LYL
